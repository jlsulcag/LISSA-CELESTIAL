...
public class JIF_NotaSalidaFar extends javax.swing.JInternalFrame {
    ...
    private void registrar() {
        if(isDatosValidos()){
            ...
            if(temp.getIdventa() != 0){
                int res = registrarDetalleVenta(temp);
    ...
    private Venta registrarVenta() {
        oVentaBl = new VentaBl();
        oVenta.setIdUsuario(oUsuario.getIdUsuario());
        ...
        oVenta.setNumComprobante(Long.parseLong(txfNumeroComprobante.getText().trim()));
...
//            }
    ...
    private void registrarDetalleKardex(Kardex oKardex) {
        for (int i = 0; i < oModeloVentaProducto.getRowCount(); i++) {
            ...
            long stockAnt = stockAnterior(oDetalleVenta.getProducto().getIdproducto(), ((Almacen) cbxOrigen.getSelectedItem()).getIdalmacen());
    ...
    private void actualizarAlmacenStock() {
        for (int i = 0; i < oModeloVentaProducto.getRowCount(); i++) {
            ...
            //Buscar por lote
            oDetalleVenta = oModeloVentaProducto.get(i);
            oAlmacenProducto = buscarAlmacenProductoXlote(oDetalleVenta.getLote(), (Almacen) cbxOrigen.getSelectedItem(), oDetalleVenta.getProducto());
    ...
    private void generarReporte(Venta obj) {
          try {
            ...
            reportPanel = reportGenric.mkReport("Ventas_NotaSalida", new String[]{"ID_VENTA"}, new Object[]{obj.getIdventa()});
