...
public class JIF_AdministrarComprobante extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        rbtnAnulado.setBackground(new java.awt.Color(204, 255, 255));
    ...
    private void tblResultadosMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblResultadosMouseReleased
        ...
        if (beanTablaPer != null) {
            //for (Paciente paciente : beanTablaPer.getPacientes()) {
    ...
    private void tblComprobantesMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblComprobantesMouseReleased
        ...
        if (beanTablaComp.getEstado()) {
            this.rbtnActivo.setSelected(true);
            this.txaMotivoAnulacion.setText("");
        } else if (beanTablaComp.getEstado() == false || beanTablaComp.getEstado() == null) {
...
//                oModeloComprobante.addAll(listComprobante);
    ...
    private void anularBoleta() {
        ...
        if (beanTablaComp != null && tblComprobantes.getSelectedRow() != -1) {
            //buscarComprobante
            //oComprobante = oBlComprobante.buscarXnumDoc(beanTabla.getNumeroBoleta(), beanTabla.getTipoComprobante());
            System.out.println("estado select " + beanTablaComp.getEstado());
            if (beanTablaComp.getEstado()) {
                oComprobante = beanTablaComp;
                if (rbtnAnulado.isSelected()) {
                    oComprobante.setEstado(Boolean.FALSE);
    ...
    private void reimprimir() {
        if (beanTablaComp != null && tblComprobantes.getSelectedRow() != -1) {
            if (beanTablaComp.getTipoComprobante().getIdtipocomprobante() == 1 || beanTablaComp.getTipoComprobante().getIdtipocomprobante() == 2) {
                if (beanTablaComp.getEstado()) {
                    try {
                        ...
                        reportPanel = reportGenric.mkReport("OtrosComprobantes", new String[]{"P_ID_BOLETA"}, new Object[]{beanTablaComp.getIdComprobante()});
