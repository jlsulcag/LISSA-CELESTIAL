...
import lissa.bl.AlergiaBl;
import lissa.bl.AnamnesisBl;
import lissa.bl.AntecedenteBl;
...
import lissa.bl.DatoAdicionalBl;
import lissa.bl.DiagnosticoBl;
import lissa.bl.ExamenAuxiliarBl;
import lissa.bl.FuncionVitalBl;
import lissa.bl.TratamientoBl;
...
import lissa.table.ModeloExamenAuxiliar;
import lissa.table.ModeloTratamiento;
...
public class JPanel_HistoriaClinica extends javax.swing.JPanel {
    ...
    private Antecedente oAntecedente = null;
    private AntecedenteBl oAntecedenteBl = null;
    private Alergia oAlergia = null;
    private AlergiaBl oAlergiaBl = null;
    private FuncionVital oFuncionVital = null;
    private FuncionVitalBl oFuncionVitalBl = null;
    private Anamnesis oAnamnesis = null;
    private AnamnesisBl oAnamnesisBl = null;
    private DatoAdicional oDatoAdicional = null;
    private DatoAdicionalBl oDatoAdicionalBl = null;
    private ModeloExamenAuxiliar oModeloExamenAuxiliar = null;
    private ExamenAuxiliar oExamenAuxiliar = null;
    private ExamenAuxiliarBl oExamenAuxiliarBl = null;
    private Tratamiento oTratamiento = null;
    private TratamientoBl oTratamientoBl = null;
    private ArrayList<Diagnostico> listDiagnostico = null;
    private ArrayList<ExamenAuxiliar> listExamenAuxiliar = null;
    private ArrayList<Tratamiento> listTratamiento = null;
    private ModeloTratamiento oModeloTratamiento = null;
    ...
    public JPanel_HistoriaClinica(JF_Principal root) {
        ...
        oModeloExamenAuxiliar = new ModeloExamenAuxiliar();
        tblExamenAuxiliar.setModel(oModeloExamenAuxiliar);
        oModeloTratamiento = new ModeloTratamiento();
        tblTratamiento.setModel(oModeloTratamiento);
        inicioHora();
        personalizaVistaTabla();
        personalizaTablaExamenAuxiliar();
    }
    ...
    private void initComponents() {
        ...
        rbtnNo.setBackground(new java.awt.Color(202, 237, 228));
        buttonGroup1.add(rbtnNo);
        rbtnNo.setText("No");

        rbtnSi.setBackground(new java.awt.Color(202, 237, 228));
        ...
        jdchFechaProxCita.setDateFormatString("dd/MM/yyyy");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        if (oAtencion != null) {
            if (oAtencion.getTipoAtencion().equals("PRIMERA ATENCIÓN")) {
...
//        fecha.setEditor(new JSpinner.DateEditor(fecha, "dd/MM/yyyy"));
    ...
    private void registrar() {
        if (oAtencion != null) {
            ...
            oAtencion.setIdUsuarioMed(userTemp.getIdUsuario());
            oAtencionBl.actualizar(oAtencion);
        }
        if (oAnamnesis == null) {
            registrarAnamnesis();
        }
        if (listDiagnostico.isEmpty()) {
            registrarDiagnostico();
        }
        if (oDatoAdicional == null) {
            registrarDatosAdicionales();
        }
        if (listExamenAuxiliar.isEmpty()) {
            registrarExamenAuxiliar();
        }
    ...
    private Atencion registrarAtencion() {
        ...
        oAtencion.setFechaAtencion(jdcFechaAtencion.getDate());
//        Date ahora = new Date(jsHora.getValue().toString());
//        SimpleDateFormat formatHora = new SimpleDateFormat("HH:mm:ss");
        SimpleDateFormat hour = new SimpleDateFormat("HH:mm:ss");
        oAtencion.setHoraAtencion(hour.format(jsHoraAtencion.getValue()));
        //oAtencion.setHoraAtencion(jsHora.getValue().toString());
        oAtencion.setHistoriaClinica(oHistoriaClinica);
        oAtencion.setIdUsuarioEnf(userTemp.getIdUsuario());
        if (oHistoriaClinica.getAtencions().isEmpty()) {
            oAtencion.setTipoAtencion("PRIMERA ATENCIÓN");
        } else {
            oAtencion.setTipoAtencion("REEVALUACION");
        }
        if (!txfEdad.getText().trim().equals("")) {
            oAtencion.setEdad(Integer.parseInt(txfEdad.getText().trim()));
        }
    ...
    private void registrarAntecedente() {
        oAntecedente = new Antecedente();
        oAntecedenteBl = new AntecedenteBl();
        oAntecedente.setAtencion(oAtencion);
        oAntecedente.setAntPersonales(txaAntPer.getText().toUpperCase().trim());
        oAntecedente.setAntFamiliares(txaAntFam.getText().toUpperCase().trim());
        oAntecedenteBl.registrar(oAntecedente);
    }

    private void registrarAlergia() {
        oAlergia = new Alergia();
        oAlergiaBl = new AlergiaBl();
        oAlergia.setAtencion(oAtencion);
        if (rbtnNo.isSelected()) {
            oAlergia.setEsAlergico(Boolean.FALSE);
        } else if (rbtnSi.isSelected()) {
            oAlergia.setEsAlergico(Boolean.TRUE);
        }
        oAlergia.setAlergias(txfAlergia.getText().toUpperCase().trim());
        oAlergia.setEstado(Boolean.TRUE);
        oAlergiaBl.registrar(oAlergia);
    }

    private void registrarFuncionVital() {
        oFuncionVital = new FuncionVital();
        oFuncionVitalBl = new FuncionVitalBl();
        oFuncionVital.setAtencion(oAtencion);
    ...
    private void registrarAnamnesis() {
        oAnamnesis = new Anamnesis();
        oAnamnesisBl = new AnamnesisBl();
        oAnamnesis.setAtencion(oAtencion);
        oAnamnesis.setAnamnesis(txaAnamnesis.getText().toUpperCase().trim());
        oAnamnesis.setMotivoConsulta(txfMotivoConsulta.getText().toUpperCase().trim());
        oAnamnesis.setExamenClinicoDirigido(txaExamenClinicoDirigido.getText().toUpperCase().trim());
        oAnamnesis.setEstado(Boolean.TRUE);
        oAnamnesisBl.registrar(oAnamnesis);
    }

    private void registrarDatosAdicionales() {
        oDatoAdicional = new DatoAdicional();
        oDatoAdicionalBl = new DatoAdicionalBl();
        oDatoAdicional.setAtencion(oAtencion);
        oDatoAdicional.setObservaciones(txaObservaciones.getText().toUpperCase().trim());
        oDatoAdicional.setProximaCita(jdchFechaProxCita.getDate());
        oDatoAdicionalBl.registrar(oDatoAdicional);
    }
    ...
    private void agregarExamenAuxiliar() {
        oExamenAuxiliar = new ExamenAuxiliar();
        oExamenAuxiliarBl = new ExamenAuxiliarBl();
        oExamenAuxiliar.setExamen(txfExamenesAuxiliares.getText().toUpperCase().trim());
        oExamenAuxiliar.setEstado(Boolean.TRUE);
        oModeloExamenAuxiliar.add(oExamenAuxiliar);
        txfExamenesAuxiliares.requestFocus();
    }

    private void personalizaTablaExamenAuxiliar() {
        Utilitarios.formateaColumna(0, tblExamenAuxiliar, 80);
    }
    ...
    private void resetComponentTab3() {
        txfExamenesAuxiliares.setText("");
        oModeloExamenAuxiliar.clear();
        oModeloTratamiento.clear();
        txaObservaciones.setText("");
        jdchFechaProxCita.setDate(null);
        txfMedico.setText("");
    }

    void cargarDatosHistorial(Atencion atencion) {
    ...
    void cargarAtencion(Atencion atencion) {
        this.oAtencion = atencion;
        jdcFechaAtencion.setDate(oAtencion.getFechaAtencion());
        //visualizar la hora en la historia clinica
        SimpleDateFormat formatoDelTexto = new SimpleDateFormat("HH:mm:ss");
        String hora = oAtencion.getHoraAtencion();
        Date horaAct = null;
        try {
    ...
    private void cargarDetalleHistoriaClinica(Atencion oAtencion) {
        //cargar datos del personal si  ya lo registro
        if (oAtencion.getIdUsuarioMed() != null && oAtencion.getIdUsuarioMed() != 0) {
            //cargar datos
            oUsuarioBl = new UsuarioBl();
        ...
        cargarAntecedente(oAtencion);
        cargarAlergia(oAtencion);
        cargarFuncionVital(oAtencion);
        cargarAnamnesis(oAtencion);
        cargarDiagnostico(oAtencion);
        cargardatoAdicional(oAtencion);
        cargarExamenAuxiliar(oAtencion);
        cargarTratamiento(oAtencion);
    }

    private void cargarAntecedente(Atencion oAtencion) {
        oAntecedente = new Antecedente();
        oAntecedenteBl = new AntecedenteBl();
        oAntecedente = oAntecedenteBl.buscar(oAtencion);
        if (oAntecedente != null) {
            txaAntPer.setText(oAntecedente.getAntPersonales());
            txaAntFam.setText(oAntecedente.getAntFamiliares());
            estadoComponentAntecedentes(false);
        } else {
            estadoComponentAntecedentes(true);
        }
    }

    private void cargarAlergia(Atencion oAtencion) {
        oAlergia = new Alergia();
        oAlergiaBl = new AlergiaBl();
        oAlergia = oAlergiaBl.buscar(oAtencion);
        if (oAlergia != null) {
            if (oAlergia.getEsAlergico()) {
                rbtnSi.setSelected(true);
            } else {
                rbtnNo.setSelected(true);
            }
            txfAlergia.setText(oAlergia.getAlergias());
            estadoComponentAlergia(false);
        } else {
            estadoComponentAlergia(true);
        }
    }

    private void cargarFuncionVital(Atencion oAtencion) {
        oFuncionVital = new FuncionVital();
        oFuncionVitalBl = new FuncionVitalBl();
    ...
    private void cargarAnamnesis(Atencion oAtencion) {
        oAnamnesis = new Anamnesis();
        oAnamnesisBl = new AnamnesisBl();
        oAnamnesis = oAnamnesisBl.buscar(oAtencion);
        if (oAnamnesis != null) {
            txfMotivoConsulta.setText(oAnamnesis.getMotivoConsulta());
            txaAnamnesis.setText(oAnamnesis.getAnamnesis());
            txaExamenClinicoDirigido.setText(oAnamnesis.getExamenClinicoDirigido());
            estadoComponentAnamnesis(false);
        } else {
            estadoComponentAnamnesis(true);
        }
    }

    private void cargardatoAdicional(Atencion oAtencion) {
        oDatoAdicional = new DatoAdicional();
        oDatoAdicionalBl = new DatoAdicionalBl();
        oDatoAdicional = oDatoAdicionalBl.buscar(oAtencion);
        if (oDatoAdicional != null) {
            txaObservaciones.setText(oDatoAdicional.getObservaciones());
            jdchFechaProxCita.setDate(oDatoAdicional.getProximaCita());
            estadoComponentDatoAdicional(false);
        } else {
            estadoComponentDatoAdicional(true);
        }
    }

    private void cargarExamenAuxiliar(Atencion oAtencion) {
        oExamenAuxiliarBl = new ExamenAuxiliarBl();
        listExamenAuxiliar = oExamenAuxiliarBl.listar(oAtencion);
        if (!listExamenAuxiliar.isEmpty()) {
            oModeloExamenAuxiliar.addAll(listExamenAuxiliar);
            estadoComponentExamenAuxiliar(false);
        } else {
            estadoComponentExamenAuxiliar(true);
        }
    }

    private void cargarTratamiento(Atencion oAtencion) {
        oTratamientoBl = new TratamientoBl();
        listTratamiento = oTratamientoBl.listar(oAtencion);
        if (!listTratamiento.isEmpty()) {
            oModeloTratamiento.addAll(listTratamiento);
            estadoComponentTratamiento(false);
        } else {
            estadoComponentTratamiento(true);
        }
    }

    private void cargarDiagnostico(Atencion oAtencion) {
        oDiagnosticoBl = new DiagnosticoBl();
        listDiagnostico = oDiagnosticoBl.buscar(oAtencion);
        if (!listDiagnostico.isEmpty()) {
            modeloDiagnostico.addAll(listDiagnostico);
            estadoComponentDiagnostico(false);
        } else {
            estadoComponentDiagnostico(true);
        }
    }

    private void registrarExamenAuxiliar() {
        if (oModeloExamenAuxiliar.size() > 0) {
            for (int i = 0; i < oModeloExamenAuxiliar.size(); i++) {
                oExamenAuxiliar = new ExamenAuxiliar();
                oExamenAuxiliarBl = new ExamenAuxiliarBl();
                oExamenAuxiliar = (ExamenAuxiliar) oModeloExamenAuxiliar.get(i);
                oExamenAuxiliar.setAtencion(oAtencion);
                oExamenAuxiliar.setEstado(Boolean.TRUE);
                oExamenAuxiliarBl.registrar(oExamenAuxiliar);
            }
        }
    }
    ...
    private void agregarTratamiento() {
        ...
        oTratamientoBl = new TratamientoBl();
        oTratamiento.setTratamiento(txfTratamiento.getText().toUpperCase().trim());
        oTratamiento.setEstado(Boolean.TRUE);
        oModeloTratamiento.add(oTratamiento);
        txfTratamiento.requestFocus();
    }

    private void registrarTratamiento() {
        if (oModeloTratamiento.size() > 0) {
            for (int i = 0; i < oModeloTratamiento.size(); i++) {
                oTratamiento = new Tratamiento();
                oTratamientoBl = new TratamientoBl();
                oTratamiento = (Tratamiento) oModeloTratamiento.get(i);
                oTratamiento.setAtencion(oAtencion);
                oTratamiento.setEstado(Boolean.TRUE);
                oTratamientoBl.registrar(oTratamiento);
            }
        }
    }
...
//            txfEnfermeria.setText(perTemp.getNombre().trim() + " " + perTemp.getApellidoPaterno().trim() + " " + perTemp.getApellidoMaterno().trim());
//        }else{
//            //debe cargar  los datos del personal de enfermeria de la base de datos
//            
//            //cargar los datos del personal medico logueado
//            txfMedico.setText(perTemp.getNombre().trim() + " " + perTemp.getApellidoPaterno().trim() + " " + perTemp.getApellidoMaterno().trim());
//        }
    ...
    private void imprimir(Atencion atencion) {
