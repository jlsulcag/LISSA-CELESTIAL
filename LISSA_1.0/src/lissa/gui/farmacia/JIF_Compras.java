...
public class JIF_Compras extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        btnProveedor.setIcon(new javax.swing.ImageIcon(getClass().getResource("/lissa/resources/search.png"))); // NOI18N
            ...
            .addGroup(jPanel6Layout.createSequentialGroup()
                ...
                .addGroup(jPanel6Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel6Layout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(txfNumeroComprobante, javax.swing.GroupLayout.DEFAULT_SIZE, 338, Short.MAX_VALUE))
                    .addGroup(jPanel6Layout.createSequentialGroup()
    ...
    private void btnProveedorKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_btnProveedorKeyPressed
        Utilitarios.requestFocusEnter(btnBuscarProducto, evt);
    ...
    private void btnProveedorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProveedorActionPerformed
        root.insertarInternalFrames(root.jifBusqProveedor);
        root.jifBusqProveedor.inicializa();
    ...
    private void txfPrecioUnitarioKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txfPrecioUnitarioKeyReleased
        if (oProducto != null) {
            if (!txfPrecioUnitario.getText().trim().equals("") && !txfCantidad.getText().trim().equals("")) {
...
//        oTipoComprobanteCtrl.rellenaTipoComprobante(cbxTipoComprobante);
//        defaultCbxTipoComprobante();
//
//    }
...
//    private void defaultCbxTipoComprobante() {
//        oTipoComprobanteCtrl = new TipoComprobanteCtrl(root);
//        oTipoComprobanteCtrl.defaultSelectCbx("boleta", cbxTipoComprobante);
//    }
//    private void cargarProveedor() {
//        oProveedorCtrl = new ProveedorCtrl(root);
//        oProveedorCtrl.Proveedor(cbxProveedor);
//    }
    ...
    private Compra registrarCompra() {
        oCompra = new Compra();
        ...
        if (oProveedor != null) {
            oCompra.setIdProveedor((int) oProveedor.getIdproveedor());
        }
        ...
        oCompra.setIdUsuarioReg(userTemp.getIdUsuario());
        oCompra = oCompraBl.registrar(oCompra);
        //Registrar los detalles de la compra
        registrarDetalleCompra(oCompra);
        return oCompra;
    }

    private void registrarDetalleCompra(Compra oCompra) {
        ...
        for (int i = 0; i < oModeloCompraProducto.getRowCount(); i++) {
            ...
            oDetalleCompraBl.registrar(oDetalleCompra);
        }
    }
...
//        }
    ...
    private Kardex registrarKardex() {
        ...
        oKardex.setAlmacenByIdDestino((Almacen) cbxAlmacen.getSelectedItem());
    ...
    private void registrarDetalleKardex(Kardex oKardex) {
        for (int i = 0; i < oModeloCompraProducto.size(); i++) {
            ...
            //Validar si el registro es unitario
            ...
            oDetalleKardex.setStockActual(Integer.parseInt((oDetalleKardex.getCantidad() + stockAnt) + ""));
            //
            int mov = oDetalleKardexBl.maxNumMov();
            oDetalleKardex.setNumeroMovimiento(mov + 1);
    ...
    private void registrarAlmacenStock() {
        for (int i = 0; i < oModeloBusqProducto.getRowCount(); i++) {
            oAlmacenProducto = new AlmacenProducto();
            oAlmacenProductoBl = new AlmacenProductoBl();
            //se establece  los items de la tabla para el registro
            oAlmacenProducto = oModeloBusqProducto.get(i);
            //buscar si existe el producto y lote para hacer el recalculo del stock
            AlmacenProducto temp = new AlmacenProducto();
            temp = oAlmacenProductoBl.buscarProductoxAlmacenyLote(oAlmacenProducto.getLote(), oAlmacenProducto.getAlmacen(), oAlmacenProducto.getProducto());

            if (temp != null && temp.getIdalmacenproducto() != 0) {//Actualizar el registro existente
                oAlmacenProducto.setIdalmacenproducto(temp.getIdalmacenproducto());
                oAlmacenProducto.setFechaRegistro(temp.getFechaRegistro());
                oAlmacenProducto.setStockActual(oAlmacenProducto.getStockActual() + temp.getStockActual());
    ...
    private void agregarDetalleCompra() {
        ...
        if (isUnitario()) {
            oDetalleCompra.setIsUnitario(true);
        } else {
            oDetalleCompra.setIsUnitario(false);
        }
        ...
        oDetalleCompra.setPrecioCompraUnitario(new BigDecimal(txfValorUnitario.getText().trim()));
        oDetalleCompra.setDescuentoItem(valor_desc_item);
        oDetalleCompra.setPrecioCompra(oDetalleCompra.getPrecioCompraUnitario().multiply(new BigDecimal(txfCantidad.getText().trim())));
    ...
    private void ingresarAlmacenProducto() {
        oAlmacenProducto = new AlmacenProducto();
        oAlmacenProducto.setProducto(oProducto);
        oAlmacenProducto.setAlmacen((Almacen) cbxAlmacen.getSelectedItem());
//        oAlmacenProducto.setStockActual(Integer.parseInt(txfCantidad.getText().trim()) * oProducto.getFraccion());
        oAlmacenProducto.setStockActual(Integer.parseInt(txfTotalUnidades.getText().trim()));
        ...
        oAlmacenProducto.setLote(txfNumeroLote.getText().trim().toUpperCase());
        oAlmacenProducto.setFechaVencimiento(jdcFechaVencimiento.getDate());
        oAlmacenProducto.setPrecioMin(new BigDecimal(txfPvMin.getText().trim()));
        oAlmacenProducto.setPrecioMax(new BigDecimal(txfPvMax.getText().trim()));
        oAlmacenProducto.setPrecioVenta(new BigDecimal(txfPrecioSugerido.getText().trim()));
        oAlmacenProducto.setUtilidadMin(Integer.parseInt(txfUtiMin.getText().trim()));
        oAlmacenProducto.setUtilidadMax(Integer.parseInt(txfUtiMax.getText().trim()));
        oModeloBusqProducto.add(oAlmacenProducto);
    }

    private void quitarAlmacenProducto(int filaSelect) {
        oModeloBusqProducto.remove(filaSelect);
    }
    ...
    private BigDecimal calcularPrecioCompraUnitario() {
        BigDecimal pcu = new BigDecimal(txfValorUnitario.getText().trim()).setScale(2, RoundingMode.HALF_UP);
        BigDecimal cant = new BigDecimal(txfCantidad.getText().trim()).setScale(2, RoundingMode.HALF_UP);
        BigDecimal fra = new BigDecimal(oProducto.getFraccion().toString()).setScale(2, RoundingMode.HALF_UP);
        return ((pcu).divide(new BigDecimal(fra.toString()), 2, RoundingMode.HALF_UP));
    }

    private void calcularPrecioVentaMin(BigDecimal pcu) {
        pvMin = (pcu.divide(CIEN.subtract(new BigDecimal(txfUtiMin.getText().trim())), 4, RoundingMode.HALF_UP)).multiply(CIEN).setScale(2, RoundingMode.HALF_UP);
        txfPvMin.setText(pvMin.toString());
    }

    private void calcularPrecioVentaMax(BigDecimal pcu) {
        pvMax = (pcu.divide(CIEN.subtract(new BigDecimal(txfUtiMax.getText().trim())), 4, RoundingMode.HALF_UP)).multiply(CIEN).setScale(2, RoundingMode.HALF_UP);
        txfPvMax.setText(pvMax.toString());
    }

    private void calcularPrecioVentaMin() {
        BigDecimal pcu = new BigDecimal(txfValorUnitario.getText().trim());
        BigDecimal utMin = new BigDecimal(txfUtiMin.getText().trim());
        pvMin = pcu.multiply((utMin.divide(new BigDecimal("100")))).add(pcu).setScale(2, RoundingMode.HALF_UP);
        txfPvMin.setText(pvMin.toString());
    }

    private void calcularPrecioVentaMax() {
        BigDecimal pcu = new BigDecimal(txfValorUnitario.getText().trim());
        BigDecimal utMax = new BigDecimal(txfUtiMax.getText().trim());
        pvMax = pcu.multiply((utMax.divide(new BigDecimal("100")))).add(pcu).setScale(2, RoundingMode.HALF_UP);
        txfPvMax.setText(pvMax.toString());
    }
    ...
    private void calcularPrecioCompraNeto() {
        ...
        // desc_unico almacena el valor del descuento en porcentaje baswado en la formula de descuento unico
        ...
        prec_compra_fraccion = valor_total_item.divide(cant, 2, RoundingMode.HALF_UP);
        //Precio de compra unitario
        prec_compra_unidad = prec_compra_fraccion.divide(fra, 4, RoundingMode.HALF_UP);
    ...
    private void calcularPreciosVenta(BigDecimal prec_compra_unidad) {
        ...
        pvMin = precioMasIGV.add(precioMasIGV.multiply(new BigDecimal(txfUtiMin.getText().trim())).divide(CIEN, 2, RoundingMode.HALF_UP)).setScale(2, RoundingMode.HALF_UP);
        txfPvMin.setText(pvMin.toString());

        pvMax = precioMasIGV.add(precioMasIGV.multiply(new BigDecimal(txfUtiMax.getText().trim())).divide(CIEN, 2, RoundingMode.HALF_UP)).setScale(2, RoundingMode.HALF_UP);
        txfPvMax.setText(pvMax.toString());
    ...
    /*
     ...
     jdcFechaCompra.getDateEditor().getUiComponent().addKeyListener(new java.awt.event.KeyListener() {
     @Override
     ...
     if (((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).getText().length() == 2) {
     ((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).setText(((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).getText() + "/");
     } else if (((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).getText().length() == 5) {
     ((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).setText(((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).getText() + "/");
     } else if (((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).getText().length() == 6) {
     if (KeyEvent.VK_ENTER == e.getKeyCode()) {
     ((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).setText(((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).getText() + annio);
     //                            cbxProveedor.requestFocus();
     }

     } else if (((JTextField) jdcFechaCompra.getDateEditor().getUiComponent()).getText().length() == 10) {
     if (KeyEvent.VK_ENTER == e.getKeyCode()) {
     //                            cbxProveedor.requestFocus();
     }
    ...
    private void buscarXRuc() {
        ...
        if (oProveedor != null) {
            txfRazonSocial.setText(oProveedor.getPersonaJuridica().getRazonSocial().trim());
        } else {
            btnProveedor.doClick();
        }
    }

    void cargaDatosProveedor(Proveedor obj) {
        oProveedor = obj;
        txfRuc.setText(oProveedor.getPersonaJuridica().getNumeroRuc().trim());
        txfRazonSocial.setText(oProveedor.getPersonaJuridica().getRazonSocial().trim());
    }
    ...
    private void calcularPrecioConIgv() {
        BigDecimal valorUnitario = new BigDecimal(txfValorUnitario.getText().trim());
        BigDecimal precioUnitario = valorUnitario.add(valorUnitario.multiply(new BigDecimal("0.18")));
        txfPrecioUnitario.setText(precioUnitario.toString());
    }

    private void calcularPrecioSinIgv() {
        BigDecimal precioUnitario = new BigDecimal(txfPrecioUnitario.getText().trim());
        BigDecimal valorUnitario = precioUnitario.divide(new BigDecimal("1.18"), RoundingMode.HALF_UP);
