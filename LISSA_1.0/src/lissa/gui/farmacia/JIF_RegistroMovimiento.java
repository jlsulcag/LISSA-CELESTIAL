...
//        oTipoComprobanteOperacionBl = new TipoComprobanteOperacionBl();
//        listTipoComprobanteOperacion = oTipoComprobanteOperacionBl.listar("");
//        cbxTipoComprobante.removeAllItems();
//        cbxTipoComprobante.addItem("<Seleccione>");
//        if (!listTipoComprobanteOperacion.isEmpty()) {
//            for (TipoComprobanteOperacion oTipoCo : listTipoComprobanteOperacion) {
//                oTipoCo.setOp(TipoComprobanteOperacion.DATO);
//                cbxTipoComprobante.addItem(oTipoCo);
//            }
//        }
//    }
    ...
    private void registrarKardexAlmacen() {
        ...
        oKardexIngreso = registrarKardexIngresoFar();
        if (oKardex != null && oKardexIngreso != null) {
            registrarDetalleKardex(oKardex);
            registrarDetalleKardex(oKardexIngreso);
        }
    ...
    private Kardex registrarKardex() {
        ...
        oKardex.setAlmacenByIdAlmacenAfectado((Almacen) cbxOrigen.getSelectedItem());
        ...
        oKardex.setPersonaOrigen(txfPersonaOrigen.getText().toUpperCase().trim());
        oKardex.setPersonaDestino(txfPersonaDestino.getText().toUpperCase().trim());
        oKardex.setObservacion(txfGlosa.getText().toUpperCase().trim());

        return oKardexBl.registrar(oKardex);
    }

    private Kardex registrarKardexIngresoFar() {
        oKardexIngreso = new Kardex();
        oKardexBl = new KardexBl();

        oKardexIngreso.setAlmacenByIdOrigen((Almacen) cbxOrigen.getSelectedItem());
        oKardexIngreso.setAlmacenByIdAlmacenAfectado((Almacen) cbxDestino.getSelectedItem());
        ...
        oKardexIngreso.setPersonaOrigen(txfPersonaOrigen.getText().toUpperCase().trim());
        oKardexIngreso.setPersonaDestino(txfPersonaDestino.getText().toUpperCase().trim());
    ...
    private void registrarDetalleKardex(Kardex oKardex) {
        ...
        for (int i = 0; i < oModeloVentaProducto.getRowCount(); i++) {
            ...
            //Buscar Stock anterior en origen
            long stockAnt = stockAnterior(oDetalleVenta.getProducto().getIdproducto(), oKardex.getAlmacenByIdOrigen().getIdalmacen());
            ...
            //
            //Buscar Stock anterior en destino
            long stockAntDestino = stockAnterior(oDetalleVenta.getProducto().getIdproducto(), oKardex.getAlmacenByIdDestino().getIdalmacen());
            oDetalleKardex.setStockAnt(Integer.parseInt(stockAntDestino + ""));
            oDetalleKardex.setStockActual(Integer.parseInt((stockAntDestino + oDetalleKardex.getCantidad()) + ""));
            //
            oDetalleKardex.setNumeroMovimiento(oDetalleKardexBl.maxNumMov() + 1);
...
//            oAlmacenProductoBl = new AlmacenProductoBl();
            ...
            //oAlmacenProducto = buscarAlmacenProductoXlote(oDetalleVenta.getLote());
...
//    private void actualizarAlmacen(DetalleVenta oDetalleVenta, Almacen almacen) {
//        actualizarOrigen(oDetalleVenta, almacen);
//        actualizarDestino(oDetalleVenta, almacen);
//    }
    private void actualizarAlmacenOrigen(DetalleVenta oDetalleVenta, Almacen almacen) {
        oAlmacenProducto = new AlmacenProducto();
        oAlmacenProductoBl = new AlmacenProductoBl();
        oAlmacenProducto = buscarProductoxAlmacenyLote(almacen, oDetalleVenta.getLote(), oDetalleVenta.getProducto());
    ...
    private void actualizarAlmacenDestino(DetalleVenta oDetalleVenta, Almacen almacen) {
        //oAlmacenProducto = new AlmacenProducto();
        oAlmacenProductoBl = new AlmacenProductoBl();
        oAlmacenProducto = buscarProductoxAlmacenyLote(almacen, oDetalleVenta.getLote(), oDetalleVenta.getProducto());
        if (oAlmacenProducto != null) {
            ...
            oAlmacenProducto.setAlmacen(almacen);
            ...
            if (oDetalleVenta.getLote() != null) {
                oAlmacenProducto.setLote(oDetalleVenta.getLote());
            } else {
            ...
            if (temp.getFechaVencimiento() != null) {
                oAlmacenProducto.setFechaVencimiento(temp.getFechaVencimiento());
            } else {
                oAlmacenProducto.setFechaVencimiento(null);
            }

            oAlmacenProducto.setPrecioMin(temp.getPrecioMin());
            oAlmacenProducto.setPrecioMax(temp.getPrecioMax());
            oAlmacenProducto.setPrecioVenta(temp.getPrecioVenta());
            oAlmacenProducto.setUtilidadMax(temp.getUtilidadMax());
            oAlmacenProducto.setUtilidadMin(temp.getUtilidadMin());
