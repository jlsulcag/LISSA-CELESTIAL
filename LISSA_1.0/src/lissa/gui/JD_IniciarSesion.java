...
public final class JD_IniciarSesion extends javax.swing.JDialog {
    ...
    private void iniciarSistemaPrincipal() {
        try {
            ...
            //jifEmitirComprobante.cargarDatosUsuario(oUsuario);
            for (UsuarioRol rolesUsuario1 : rolesUsuario) {
                oRol = oBlRol.buscarId(rolesUsuario1.getRol().getIdrol());
                switch (oRol.getDenominacion().trim()) {
                    case "ADMINISTRADOR":
                    ...
                    case "CAJA":
                        ...
                        break;
                    case "ENFERMERIA":
                        root.configuraInterfazEnfermeria();
                        break;
                    case "MEDICO":
                        root.configuraInterfazEnfermeria();
                        break;
    ...
    private void iniciarSistemaxRol(Rol rol) {
        switch (rol.getDenominacion().trim()) {
            ...
            case "SISTEMAS":
                ...
                break;
            case "OCUPACIONAL":
                root.configuraInterfazOcupacional();
                break;
            case "JEFE CAJA":
