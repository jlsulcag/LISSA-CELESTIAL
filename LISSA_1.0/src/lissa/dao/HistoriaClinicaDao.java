...
public class HistoriaClinicaDao {
    ...
    public HistoriaClinica buscar(int idpersona) {
     ...
     public HistoriaClinica buscar(Persona per, TipoHistoriaClinica thc) {
        try {
            iniciarOperacion();
            String hql = "select hc from HistoriaClinica hc inner join fetch hc.persona p inner join fetch hc.tipoHistoriaClinica thc where p.idPersona=" + per.getIdPersona() + " and thc.idtipohc = "+thc.getIdtipohc();
            Query query = sesion.createQuery(hql);
            bean = (HistoriaClinica) query.uniqueResult();
        } catch (HibernateException he) {
            manejaExcepcion(he);
